...
class User:
    ...
    def __init__(self, path="user.config"):
        ...
        self.data = DFSsysData(path=path)
        ...
        self.data_struct = DataStructures(self.data.basic_params)
        ...
        self.udp_receive_socket.bind((self.data.basic_params['IP_ADDR'], self.data.basic_params['UDP_Receive_port']))
    ...
    def setup_main_gui(self):
        app = QApplication([])
        self.UI = DFSsysGUI(self.data.log_info, self.data_struct.onlines, self.data_struct.duplicate_packets, self.lock)
    ...
    def trigger_UI(self):
        while True:
            ...
            time.sleep(self.data.basic_params['GUI_update_rate'] / 1000)  # trigger the update every 100 ms
    ...
    def check_onlines(self):
        while True:
            if self.close_event.is_set():
                break
            with self.lock:
                self.data_struct.check_onlines_data(self.data.basic_params['Removal_margin'])
            time.sleep(self.data.basic_params['Data_check_rate'] / 1000)
        return
    ...
    def o_packet_proc(self, p):
        ...
        self.data_struct.add_item_onlines(p)
        self.data_struct.add_item_duplicate_packets(p)  # for testing only

    def req_packet_proc(self, p):
        print('got req packet %s' % self.data.basic_params['IP_ADDR'])

    def res_packet_proc(self, p):
        print('got res packet %s' % self.data.basic_params['IP_ADDR'])
    ...
    def online_packet_thread(self):
        while True:
            ...
            with self.lock:
                ...
                self.data.basic_params['packet_counter'] = (self.data.basic_params['packet_counter'] + 1) % (2 ** 32)
    ...
    def udp_transmit_thread(self):
        while True:
            ...
            with self.lock:
                ...
                self.udp_transmit_socket.sendto(pk.dumps(p), (self.data.basic_params['Broadcast_addr'],
            ...
            try:
                ...
                with self.lock:
                    if self.data.udp_receive_queue.full():
                        ...
                        continue
                    if self.data_struct.should_process_packet(data):
